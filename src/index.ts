...
export default defineIndicator({
    ...
    async calculate(context) {
        ...
        if (index < parameters.emaSlow) return;
        ...
        const tf15 = await context.requestHistory(context.symbol, '15');
        const tf60 = await context.requestHistory(context.symbol, '60');
        ...
        if (isBull) context.setBarBackgroundColor(index, '#00808030');
        if (isBear) context.setBarBackgroundColor(index, '#ff000030');
